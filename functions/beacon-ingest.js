/ Beacon Ingestion Worker - Autonomous Optimization System // Processes user behavior data and stores it in Supabase import { createClient } from '@supabase/supabase-js';

// CORS Configuration const corsHeaders = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Requested-With', 'Access-Control-Max-Age': '86400', };

export default { async fetch(request, env, ctx) { // 1. Handle Preflight OPTIONS request immediately if (request.method === 'OPTIONS') { return new Response(null, { headers: corsHeaders, status: 204, }); }
...
try {
  ...
  if (!supabaseUrl || !supabaseKey) {
    ...
    // Return 500 but WITH CORS headers so the frontend sees the error, not a CORS failure
  ...
  // Critical Error Response - STILL returns CORS headers so browser doesn't hide the error
  return new Response(JSON.stringify({ 
    success: false, 
...
// Helper function to determine device type from user agent function getDeviceType(userAgent) { if (!userAgent) return 'desktop';

const ua = userAgent.toLowerCase(); if (/mobile|android|iphone|ipod|blackberry|opera mini|iemobile/i.test(ua)) { return 'mobile'; } else if (/tablet|ipad|playbook|silk/i.test(ua)) { return 'tablet'; } return 'desktop'; } }
